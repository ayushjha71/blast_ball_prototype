...
public class Slot : MonoBehaviour
{
    ...
    private ColorCounter mCounterPrefab;
    ...
    public void FillSlot(Color color, int count)
    {
        ...
        if(mCounterPrefab == null)
        {
            mCounterPrefab = FindObjectOfType<ColorDisplay>().CounterPrefab;
        }

        if (mCounter == null && mCounterPrefab != null)
        {
            mCounter = Instantiate(mCounterPrefab, transform.position + new Vector3(0.3f, 0.3f, 0), Quaternion.identity, transform);
        }
